...
public class DBHandler {
    ...
    public static Person validateLogin(String username, String password) throws ConnectException {
        ...
        StringBuilder contentUser = dbAPICallGet("https://com-iv1201-server.herokuapp.com/user/" + username,
                myJsonObj.getString("access_Token"));
        JSONObject myJsonObj2 = new JSONObject(contentUser.toString());
        Person person = new Person(myJsonObj2.getInt("id"), myJsonObj2.getString("name"),
                myJsonObj.getString("access_Token"), myJsonObj2.getJSONObject("role").getString("name"));
    ...
    public static String validateEmail(String email) throws ConnectException {
        ...
        StringBuilder content = dbAPICallPostAuth("https://com-iv1201-server.herokuapp.com/resetAccount/getToken",
                body);
        if (content == null)
            return null;
    ...
    public static List<Competence> loadCompetence(String language) {
        ...
        for (int i = 0; i < (myJsonArray.length() / map.size()); i++) {
            competenceList.add(new Competence(myJsonArray.getJSONObject(i + j).getInt("id"),
                    myJsonArray.getJSONObject(i + j).getString("name")));
        }
